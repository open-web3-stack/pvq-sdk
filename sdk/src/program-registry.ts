...
export function parseMetadata(
  hexOrJSON: `0x${string}` | Record<string, unknown> | ProgramMetadata | Uint8Array,
...
export function parseFunctionMetadata(func: FunctionMetadata, registry: TypeRegistry) {
  ...
  const returnType = func.output.toNumber() !== 0 ? registry.lookup.getTypeDef(func.output.toNumber()) : null
...
export class ProgramRegistry {
  ...
  constructor(programMetadata: Record<string, unknown>) {
    ...
    this.entrypoints = this.metadata.entrypoints.map((entrypoint, index) => this.createEntrypoint(entrypoint, index))
